...
(function (ns, undefined) {
	...
	function getKey(line, parentLevel) {
		return cleanupLine(getKeyFromLine(line.replace(getNumDots(parentLevel), '')));
	}
	...
	function c2(str) {
		...
		if (
			_.every(substr, function (character) {
				return character === '.';
			})
		) {
	...
	function countStartingDots(str) {
		var differs = _.find(str, function (character) {
			return character !== '.';
		});
		if (differs) {
		...
		if (
			_.every(str, function (character) {
				return character === '.';
			})
		) {
			return str.length;
	...
	ns.util = {
		...
		specialAttributes: (function () {
			if (!!SOSI.types) {
				return _.reduce(
					SOSI.types,
					function (attrs, type, key) {
						if (_.isObject(type[1])) {
							// true for complex datatypes
	...
	_.each(ns.koordsysMap, function (koordsys) {
		...
		} else if (!_.isUndefined(window.Proj4js)) {
			//older proj4js (=< 1.1.0)
...
(function (ns, undefined) {
	...
	function parseBbox(data) {
		...
		return [parseFloat(ll[1]), parseFloat(ll[0]), parseFloat(ur[1]), parseFloat(ur[0])];
	}
...
(function (ns, undefined) {
	...
	ns.Point = ns.Base.extend({
		...
		initialize: function (line, origo, unit) {
			if (_.isNumber(line)) {
				/* initialized directly with x and y */
				this.x = line;
				this.y = origo;
				return;
			}
	...
	ns.LineStringFromArc = ns.LineString.extend({
		// BUEP - an arc defined by three points on a circle
		initialize: function (lines, origo, unit) {
...
(function (ns, undefined) {
	...
	ns.Feature = ns.Base.extend({
		...
		parseData: function (data, origo, unit) {
			...
			this.attributes = _.reduce(
				this.attributes,
				function (attrs, value, key) {
...
(function (ns, undefined) {
	...
	var SosiData = ns.Base.extend({
		initialize: function (data) {
			...
			this.features = new ns.Features(_.omit(data, ['HODE', 'HODE 0', 'DEF', 'OBJDEF', 'SLUTT']), this.hode);
		},
...
if (!(typeof require == 'undefined')) {
	...
	if (encoding && encoding !== 'UTF8') {
		/* if unlike UTF8, we need iconv, but only then */
